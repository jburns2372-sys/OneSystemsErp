...
router.post('/explainPayslipWithAI', async (req, res) => {
  try {
    const { payrollId } = req.body;

    if (!payrollId) {
      return res.status(400).json({ success: false, error: 'Payroll ID is required.' });
    }
    ...
    console.error('Error in explainPayslipWithAI:', error);
