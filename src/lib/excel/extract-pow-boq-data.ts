    ...
    if (descCell) {
      ...
      const item = await prisma.bOQExtractedItem.create({
        data: {
          ...
          materialUnitCost: matCost ? Number(matCost) : null,
          laborUnitCost: labCost ? Number(labCost) : null,
          equipmentUnitCost: eqCost ? Number(eqCost) : null,
          totalDirectCost: totalDirect ? Number(totalDirect) : null,
          ocm: ocm ? Number(ocm) : null,
          cp: cp ? Number(cp) : null,
          vat: vat ? Number(vat) : null,
          totalIndirectCost: totalIndirect ? Number(totalIndirect) : null,
