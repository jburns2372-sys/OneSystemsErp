...
import { executeFinanceApprovalMutation } from '@/lib/services/schedule-gateway';
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string, scheduleId: string }> }) {
  ...
  const access = await checkUserAccess(session.id, _authParams.id, 'Scheduling', 'APPROVE');
  if (!access.allowed) {
  ...
  try {
    ...
    const access = await checkSchedulingAccess(actor.id, actor.role, projectId, 'canApprove'); // Enforced as per correction 1

    if (!access.allowed) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 403 });
    }

    const result = await executeFinanceApprovalMutation({
      projectId,
      scheduleId,
      expectedRowVersion,
    ...
    console.error('Error approving finance review:', error);
